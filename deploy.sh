...
Example:
    sh deploy.sh scratch_gui 192.168.123.123 8601:8601
"
	exit 0
fi
...
echo "xiaoxiao" | ssh -tt zx@$TARGET '
cd /opt/soft/'"$IMAGE"';
sudo docker container stop $(docker container ls -aq -f "ancestor='"$IMAGE"'");
sudo docker load --input '"$IMAGE"'.tar.gz;
sudo docker run -d -p '"$PORT"' '"$IMAGE"';
sudo docker container rm $(docker container ls -aq -f "status=exited");
sudo docker rmi $(docker images -f "dangling=true" -q --no-trunc);
'
